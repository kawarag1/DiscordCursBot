...
from app.src.security.oauth import get_current_owner, get_current_owner_from_refresh
...
@router.post("/get_owner", description="Авторизация владельца", response_model=AccessToken)
...
async def refresh_tokens(response: Response, owner: OwnerSchema = Depends(get_current_owner_from_refresh), session: AsyncSession = Depends(get_session), redis = Depends(get_redis(0))):
